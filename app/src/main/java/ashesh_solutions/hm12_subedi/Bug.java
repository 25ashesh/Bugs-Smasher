...
public class Bug {
    ...
    public void birth (Canvas canvas) {
        ...
        // Check if bug is alive yet
            ...
            if (curTime - startBirthTimer >= timeToBirth) {
                ...
                if (x < Assets.roach.getWidth()/2)
                    x = Assets.roach.getWidth()/2;
                else if (x > canvas.getWidth() - Assets.roach.getWidth()/2)
                    x = canvas.getWidth() - Assets.roach.getWidth()/2;
                y = 0;
    ...
    public void move (Canvas canvas) {
        ...
        if (state == BugState.Alive) {
            ...
            long alternatingTime = System.currentTimeMillis() / 100 % 10;
            if (alternatingTime % 2 == 0)
                // Draw bug on screen
                canvas.drawBitmap(Assets.roach, x-Assets.roach.getWidth()/2,  y-Assets.roach.getHeight()/2, null);
            else
                // ADD CODE HERE - Draw each frame of animation as appropriate - don't just draw 1 frame
                canvas.drawBitmap(Assets.roach_moved, x-Assets.roach_moved.getWidth()/2,  y-Assets.roach_moved.getHeight()/2, null);
    ...
    public boolean touched (Canvas canvas, int touchx, int touchy) {
        ...
        if (state == BugState.Alive) {
            ...
            if (dis <= Assets.roach.getWidth()*0.5f) {
    ...
    public void drawDead (Canvas canvas) {
        if (state == BugState.DrawDead) {
            canvas.drawBitmap(Assets.roach_dead, x-Assets.roach_dead.getWidth()/2,  y-Assets.roach_dead.getHeight()/2, null);
